...
df1$Species <- gsub('/'," f.sp. ",df1$Species,ignore.case=F)
df1$Species <- gsub('fsp'," f.sp.",df1$Species,ignore.case=F)
df1$Species <- gsub('F\\.',"F. ",df1$Species,ignore.case=F)
df1$Species <- gsub(' CBS.*',"",df1$Species,ignore.case=F)
df1$Species <- gsub(" Fus2.*","",df1$Species,ignore.case=F)
df1$Species <- gsub(" PHW808.*","",df1$Species,ignore.case=F)
df1$Species <- gsub(" 4287.*","",df1$Species,ignore.case=F)
...
df1$Species <- gsub(" Tu67.*","",df1$Species,ignore.case=F)
df1$Species <- gsub(" FOP1.*","",df1$Species,ignore.case=F)
df1$Species <- gsub(" 5176.*","",df1$Species,ignore.case=F)
...
ggsave(filename, plot = facet_species, width =10, height = 15, units = "cm", limitsize = FALSE)
#
#
# #---
# #
