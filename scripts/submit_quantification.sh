...
usearch -otutab $CurDir/$QueryReads -db $CurDir/$RefDb -strand plus -id $Identity -biomout ${Prefix}_${OtuType}_table.biom -otutabout ${Prefix}_${OtuType}_table.txt -notmatched ${Prefix}_${OtuType}_nomatch.fa -userout ${Prefix}_${OtuType}_hits.out -userfields query+target
# Normalise results to 10000 reads
usearch -otutab_rare ${Prefix}_${OtuType}_table.txt -sample_size 1000 -output ${Prefix}_${OtuType}_table_norm.txt

# Combine reads by species and filter taxa with reads fewer than a given threshold
# These steps were not used in final use
...
# ProgDir=/home/deakig/usr/local/bin
# $ProgDir/usearch -otutab_rare tmp.txt -sample_size 1000 -output tmp2.txt
