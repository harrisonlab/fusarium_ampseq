...
# df1_onion <- read.delim("~/Downloads/AHDB_new/plate3/quantified/mix-A/onion/mix-A_16S_zOTUs_zOTUs_table.txt")
...
df1_onion$Genus <- sapply( x, "[", 2)
df1_onion$Genus <- gsub("Gp","Unkown group ",df1_onion$Genus,ignore.case=F)
df1_onion$Genus <- gsub("_"," ",df1_onion$Genus,ignore.case=F)
df1_onion$Genus <- gsub("genera incertae sedis","(incertae sedis)",df1_onion$Genus,ignore.case=F)

# Read in daffodil table
df1_daffodil <- read_delim(f2, "\t", escape_double = FALSE, trim_ws = TRUE)
# df1_daffodil <- read.delim("~/Downloads/AHDB_new/plate3/quantified/mix-A/daffodil/mix-A_16S_zOTUs_zOTUs_table.txt")
...
df1_daffodil$Genus <- sapply( x, "[", 2)
df1_daffodil$Genus <- gsub("Gp","Unkown group ",df1_daffodil$Genus,ignore.case=F)
df1_daffodil$Genus <- gsub("_"," ",df1_daffodil$Genus,ignore.case=F)
df1_daffodil$Genus <- gsub("genera incertae sedis","(incertae sedis)",df1_daffodil$Genus,ignore.case=F)

# Read in stocks table
df1_stocks <- read_delim(f3, "\t", escape_double = FALSE, trim_ws = TRUE)
# df1_stocks <- read.delim("~/Downloads/AHDB_new/plate3/quantified/mix-A/stocks/mix-A_16S_zOTUs_zOTUs_table.txt")
...
df1_stocks$Genus <- sapply( x, "[", 2)
df1_stocks$Genus <- gsub("Gp","Unkown group ",df1_stocks$Genus,ignore.case=F)
df1_stocks$Genus <- gsub("_"," ",df1_stocks$Genus,ignore.case=F)
df1_stocks$Genus <- gsub("genera incertae sedis","(incertae sedis)",df1_stocks$Genus,ignore.case=F)
...
library(reshape2)
df2 <- melt(df1_total, id.vars=c('OTU ID', 'Genus'),value.name = "Counts", variable.name='Run')
...
df6 <- summarySE(df5[ which(df5$Total > 1000),], measurevar="norm", groupvars=c(c("Mix", "Field", "Locus", "Genus")))
