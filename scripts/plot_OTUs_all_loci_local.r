...
opt_list = list(
    ...
    make_option("--threshold", type="character", help="Threshold for presence/absence set as numeric or 'none'")
)
...
df3 <- data.frame(df2)
df3$group <- gsub("_ITS$|_TEF$|_SIX13$|_OG12981$|_OG13890$","",rownames(df3))
df3$group <- gsub("_.$","",df3$group)
...
df6$reps <- ifelse(grepl("pooled",df6$Run), "pooled","tech. reps.")
...
#  facet_species
#} else {
#  facet_species <- facet_species + geom_hline(mapping = NULL, data = NULL, yintercept = threshold, linetype = 2)
#  facet_species
#}
