...
# Read data:
df1 <- read.delim2('/Volumes/GGB/Harrison/Projects/AHDB-FOC-2019-C300106/Science/Obj4-Establish-Potential-AmpliconSeq/metabarcoding_data/3\ taxon\ abundance\ tables/comparative\ soil\ types/OG4952/OG4952_soil_samples_no-mismatches_zOTUs_table.txt'
...
prefix <- "comparative_soil_types_OG4952"
filename <- paste(prefix, 'facet_species.pdf', sep='_')
ggsave(filename, plot = facet_Genus, width =30, height = fig_height, units = "cm", limitsize = FALSE)
...
ggsave(filename, plot = facet_Genus, width =10, height = fig_height, units = "cm", limitsize = FALSE)
}

# Plot total reads
df8 <- summarySE(df5, measurevar="Counts", groupvars=c(c("Mix", "Field", "Locus", "Species", "Dilution")))

df8$id = numeric(nrow(df8))
for (i in 1:nrow(df8)){
   dat_temp <- df8[1:i,]
   df8[i,]$id <- nrow(dat_temp[dat_temp$Species == df8[i,]$Species,])
 }
# Create a data frame that can be used tofilter genera based on abundance

dfx <- dcast(df8, id~Species, value = 'value', value.var = 'Counts')
dfx$id <- NULL
dfx <- data.frame(apply(dfx, 2, sort, decreasing=T))
y <- as.logical(dfx[1,] > 10)
dfx <- data.frame(t(dfx))
...
df8$SpeciesMod <- gsub("[^[:alnum:] ]", ".", df8$Species)
df8$SpeciesMod <- gsub(" ", ".", df8$SpeciesMod)

dfy <- merge(dfx, df8, by="SpeciesMod")

df9 <- dfy[ which(dfy$keep == TRUE),]

factor(df9$Field)
df9$Field <- factor(df9$Field, levels = c("S1", "S2", "S3", "S4"))
df9$Dilution <- gsub("D1","control",df9$Dilution,ignore.case=F)
df9$Dilution <- gsub("D2","1x10^2",df9$Dilution,ignore.case=F)
df9$Dilution <- gsub("D3","1x10^3",df9$Dilution,ignore.case=F)
df9$Dilution <- gsub("D4","1x10^4",df9$Dilution,ignore.case=F)
df9$Dilution <- gsub("D5","1x10^5",df9$Dilution,ignore.case=F)
df9$Dilution <- gsub("D6","1x10^6",df9$Dilution,ignore.case=F)
factor(df9$Dilution)
df9$Dilution <- factor(df9$Dilution, levels = c("control", "1x10^2", "1x10^3", "1x10^4", "1x10^5", "1x10^6"))


facet_Genus<-ggplot(data=subset(df9), aes(x=Species, y=Counts))
# facet_Genus <- facet_Genus + scale_y_continuous(limits = c(0, 1000))
facet_Genus <- facet_Genus + geom_bar(stat="identity")
facet_Genus <- facet_Genus + theme(axis.text.x=element_text(angle = -45, hjust = 0))
...
fig_height <- (length(unique(df2$Dilution))*5)+10
prefix <- "comparative_soil_types_OG4952"
filename <- paste(prefix, 'facet_species_totals.pdf', sep='_')
ggsave(filename, plot = facet_Genus, width =30, height = fig_height, units = "cm", limitsize = FALSE)

for (soil in levels(df9$Field)) {
  facet_Genus<-ggplot(data=subset(df9, df9$Field == soil), aes(x=Species, y=Counts))
  # facet_Genus <- facet_Genus + scale_y_continuous(limits = c(0, 1000))
  facet_Genus <- facet_Genus + geom_bar(stat="identity")
  facet_Genus <- facet_Genus + theme(axis.text.x=element_text(angle = -45, hjust = 0))
  facet_Genus <- facet_Genus + ylab('Total read count') + xlab('')
  ...
  fig_height <- (length(unique(df2$Dilution))*5)+10
  prefix <- "comparative_soil_types_OG4952"
  filename <- paste(prefix, soil ,'facet_species_totals.pdf', sep='_')
  ggsave(filename, plot = facet_Genus, width =10, height = fig_height, units = "cm", limitsize = FALSE)
}
