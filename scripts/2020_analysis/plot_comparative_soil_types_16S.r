...
# Read data:
df1 <- read.delim2('/Volumes/GGB/Harrison/Projects/AHDB-FOC-2019-C300106/Science/Obj4-Establish-Potential-AmpliconSeq/metabarcoding_data/3\ taxon\ abundance\ tables/comparative\ soil\ types/16S/16S_soil_samples_zOTUs_table.txt'
...
df1$Genus <- gsub("Gp","Unknown group ",df1$Genus,ignore.case=F)
df1$Genus <- gsub("_"," ",df1$Genus,ignore.case=F)
df1$Genus <- gsub("genera incertae sedis","(incertae sedis)",df1$Genus,ignore.case=F)

df1$OTU <- sapply( x, "[", 1)
df1$Genus = ifelse(df1$Genus == 'unknown', paste("unknown (", df1$OTU, ")", sep = ""), df1$Genus)
df1$OTU <- NULL
...
df3 <- aggregate(df2$Counts, by=list(df2$Genus, df2$Mix, df2$Field, df2$Dilution, df2$Rep, df2$Locus), FUN=sum)
colnames(df3) <- c("Genus", "Mix", "Field", "Dilution", "Rep", "Locus", "Counts")
...
df6 <- summarySE(df5[ which(df5$Total > 1000),], measurevar="norm", groupvars=c(c("Mix", "Field", "Locus", "Genus", "Dilution")))
...
prefix <- "comparative_soil_types_16S"
filename <- paste(prefix, 'facet_genus.pdf', sep='_')
ggsave(filename, plot = facet_Genus, width =80, height = fig_height, units = "cm", limitsize = FALSE)



for (soil in levels(df7$Field)) {
facet_Genus<-ggplot(data=subset(df7, df7$Field == soil), aes(x=Genus, y=norm))
# facet_Genus <- facet_Genus + scale_y_continuous(limits = c(0, 1000))
facet_Genus <- facet_Genus + geom_bar(stat="identity")
facet_Genus <- facet_Genus + theme(axis.text.x=element_text(angle = -45, hjust = 0))
facet_Genus <- facet_Genus + ylab('Read count (per 1000 mapped reads)') + xlab('')
...
fig_height <- (length(unique(df2$Dilution))*5)+10
prefix <- "comparative_soil_types_16S"
filename <- paste(prefix, soil, 'facet_genus.pdf', sep='_')
