...
# Read data:
# df1 <- read_delim(/Volumes/GGB/Harrison/Projects/AHDB-FOC-2019-C300106/Science/Obj4-Establish-Potential-AmpliconSeq/metabarcoding_data/3 taxon abundance tables/FOC FON FOM soils/TEF/TEF_2019_samples_no-mismatches_zOTUs_table.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
df1 <- read.delim("/Volumes/GGB/Harrison/Projects/AHDB-FOC-2019-C300106/Science/Obj4-Establish-Potential-AmpliconSeq/metabarcoding_data/3 taxon abundance tables/FOC FON FOM soils/OG13890/OG13890_2019_samples_no-mismatches_zOTUs_table.txt")
...
df1$OTU <- sapply( x, "[", 1)
df1$Species[df1$Species == "unknown"] <- df1$OTU
df1$OTU <- NULL
...
df6 <- summarySE(df5[ which(df5$Total > 100),], measurevar="norm", groupvars=c(c("Mix", "Field", "Locus", "Species", "Plate")))
...
df7$Species <- gsub("OTU454","unknown (OTU454)",df7$Species,ignore.case=F)
